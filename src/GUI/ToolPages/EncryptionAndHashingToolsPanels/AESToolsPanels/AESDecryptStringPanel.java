...
public class AESDecryptStringPanel extends ContentPanelBase {
	...
	JLabel encryptedStringLabel = new JLabel("Text to be AES decrypted: ");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == startButton && !encryptedString.getText().isBlank()) {
			...
			String decryptedText = "Encrypted text: ";
			...
			if(!password.getText().trim().equals("")) {
				...
				try {
					decryptedCipherText = aes.decrypt(algorithm, encryptedString.getText(), key, ivParameterSpec);
				} catch (InvalidKeyException | NoSuchPaddingException | NoSuchAlgorithmException
						| InvalidAlgorithmParameterException | BadPaddingException | IllegalBlockSizeException e1) {
					e1.printStackTrace();
				}
				decryptedText += decryptedCipherText;
			}else {
			    IvParameterSpec ivParameterSpec = aes.generateIv();
			    SecretKey key = null;
				try {
			    ...
			    String decryptedCipherText = null;
				try {
					decryptedCipherText = aes.decryptPasswordBased(encryptedString.getText(), key, ivParameterSpec);
				} catch (InvalidKeyException | NoSuchPaddingException | NoSuchAlgorithmException
						| InvalidAlgorithmParameterException | BadPaddingException | IllegalBlockSizeException e1) {
					e1.printStackTrace();
				}
			...
			JOptionPane.showMessageDialog(null, decryptedText, "Decrypt text", JOptionPane.INFORMATION_MESSAGE);
			...
			error.setText("Type a text to decrypt!");
