...
public class GetPathsInFolderPanel extends ContentPanelBase implements ActionListener {

	private FileExtension fileExtension = new FileExtension();
	private FindFiles findFiles = new FindFiles();
	private OutputPaths outputPaths = new OutputPaths();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == startButton && sourcefile != null && destinationfile  != null && !newNameTextField.getText().isBlank()) {
			...
			if(extensions.getText().isBlank() || extensions.getText().equals("*")) {
				...
				inputExtensions = Arrays.asList(extensions.getText().split(","));
			        ...
			        if (!(validExtensions.size() > 0)) {
			            ...
			            JOptionPane.showMessageDialog(null, "Inputted file extensions are not valid.", "Get Folder paths", JOptionPane.ERROR_MESSAGE);
			        } else {
			...
			if(fileStrings.size() > 0) {
				try {
					outputPaths.outputPaths(fileStrings, save, sourcefile.getAbsolutePath(), destinationfile.getAbsolutePath(), newNameTextField.getText());
		            JOptionPane.showMessageDialog(null, "Successfully created txt-file containing \n" + sourcefile.getAbsolutePath() + " File paths", "Get Folder paths", JOptionPane.INFORMATION_MESSAGE);
				} catch (IOException e1) {
					e1.printStackTrace();
				}
			} else {
	            JOptionPane.showMessageDialog(null, "No path found in Folder " + sourcefile.getAbsolutePath(), "Get Folder paths", JOptionPane.ERROR_MESSAGE);
			}
