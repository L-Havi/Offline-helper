...
public class SplitPDFFilePanel extends ContentPanelBase {
	...
	SplitPdfAction splitpdf = new SplitPdfAction();
		...
		if(e.getSource() == startButton && file != null && pdfFile != null && name != null) {
			...
			if(pagesArray == null && (pages.getText() != null || !pages.getText().isBlank())) {
				...
				try {
					...
					splitpdf.split(pdfFile.getAbsolutePath(), file.getAbsolutePath(), name, pagesArray);
					JOptionPane.showMessageDialog(null, "Successfully split PDF to " + file.getAbsolutePath(), "Split PDF Files", JOptionPane.INFORMATION_MESSAGE);
				} catch (IOException e1) {
					JOptionPane.showMessageDialog(null, "Failed to split PDF to " + file.getAbsolutePath(), "Split PDF Files", JOptionPane.ERROR_MESSAGE);
					e1.printStackTrace();
				}
